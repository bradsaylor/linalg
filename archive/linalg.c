...
struct Matrix_list
{ // Data structure to keep list of allocated matrices
...
struct Matrix* alloc_mat(char* name, int num_rows, int num_columns, float* elements);
struct Matrix_list* matrix_list_add(struct Matrix_list* mat_list, struct Matrix* new_matrix);
int print_matrix(struct Matrix* matrix);
int free_matrix_list(struct Matrix_list* mat_list);
struct Matrix_list* add_test_data(float* elements, char* name, int num_rows, int num_cols);
...
struct Test_data test_data[] = {
    {.name = test_name1, .elements = test_elements1, .num_rows = 5, .num_cols = 1},

    {.name = test_name2, .elements = test_elements2, .num_rows = 5, .num_cols = 1},

    {.name = test_name3, .elements = test_elements3, .num_rows = 5, .num_cols = 1},

    {.name = test_name4, .elements = test_elements4, .num_rows = 5, .num_cols = 1},

    {.name = test_name5, .elements = test_elements5, .num_rows = 5, .num_cols = 1}};
...
int main()
{
    ...
    for (int i = 0; i < 2; i++)
    {
        ...
        matrix_list = add_test_data(temp.elements, temp.name, temp.num_rows, temp.num_cols);
    }
...
struct Matrix_list* matrix_list_add(struct Matrix_list* mat_list, struct Matrix* new_matrix)
{
    ...
    {
        mat_list = realloc(mat_list, mat_list->size + MAT_LIST_SIZE_INC * sizeof(struct Matrix*));
